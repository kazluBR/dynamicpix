...
class editor {
  ...
  #createSvgDocument() {
    ...
    svg.setAttribute(
      'height',
      this.#size * (this.#height + 3) + TRANSLATE_Y + SQUARE_COLOR_SIZE * 1.5,
    )
  ...
  #createLine(orientation, i) {
    ...
    if (i % this.#gridLength == 0) line.setAttribute('stroke-width', '2')
  ...
  #changeCalculatedSize(orientation) {
    ...
    calculated.setAttribute('font-size', this.#size)
    if (orientation == 0) calculated.setAttribute('x', (this.#width + 0.5) * this.#size)
    else calculated.setAttribute('y', (this.#height + 1) * this.#size)
  }
      ...
      for (let j = data.points[0].length - 1; j >= 0; j--) {
        if (counting) {
          ...
          if (data.points[i][j] == 0 || data.points[i][j] != aux) {
            if (data.points[i][j] == 0) counting = false
            horizontalNumbers[i].push({ number: count, color: aux })
            count = 0
          }
        } else if (data.points[i][j] > 0 && data.points[i][j] != aux) counting = true
        aux = data.points[i][j]
      }
      if (counting) {
        count++
      ...
      for (let j = data.points.length - 1; j >= 0; j--) {
        if (counting) {
          ...
          if (data.points[j][i] == 0 || data.points[j][i] != aux) {
            if (data.points[j][i] == 0) counting = false
            verticalNumbers[i].push({ number: count, color: aux })
            count = 0
          }
        } else if (data.points[j][i] > 0 && data.points[j][i] != aux) counting = true
        aux = data.points[j][i]
      }
      if (counting) {
        count++
  ...
  #highlightSquare(evt) {
    ...
    if (i < this.#width && j < this.#height) {
      ...
      if (i % this.#gridLength != 0) verticalLine.setAttribute('stroke-width', '2')
      i = i + 1
      ...
      if (j % this.#gridLength != 0) horizontalLine.setAttribute('stroke-width', '2')
      j = j + 1
  ...
  #fadeSquare(evt) {
    ...
    if (i < this.#width && j < this.#height) {
      ...
      if (i % this.#gridLength != 0) verticalLine.setAttribute('stroke-width', '1')
      i = i + 1
      verticalLine = document.getElementById('line_0.' + i)
      verticalLine.setAttribute('stroke', LINE_COLOR)
      if (i % this.#gridLength != 0) verticalLine.setAttribute('stroke-width', '1')
      ...
      if (j % this.#gridLength != 0) horizontalLine.setAttribute('stroke-width', '1')
      j = j + 1
      horizontalLine = document.getElementById('line_1.' + j)
      horizontalLine.setAttribute('stroke', LINE_COLOR)
      if (j % this.#gridLength != 0) horizontalLine.setAttribute('stroke-width', '1')
  ...
  #decreaseSize(evt) {
    ...
    if (orientation == 0) {
      ...
      if (this.#height < MIN_DIMENSION) this.#height += this.#gridLength
    } else if (orientation == 1) {
      this.#width -= this.#gridLength
      if (this.#width < MIN_DIMENSION) this.#width += this.#gridLength
    } else {
      this.#height -= this.#gridLength
      if (this.#height < MIN_DIMENSION) this.#height += this.#gridLength
      this.#width -= this.#gridLength
  ...
  #increaseSize(evt) {
    ...
    if (orientation == 0) {
      this.#height += this.#gridLength
      if (this.#height > MAX_HEIGTH_DIMENSION) this.#height -= this.#gridLength
    } else if (orientation == 1) {
      this.#width += this.#gridLength
      if (this.#width > MAX_WIDTH_DIMENSION) this.#width -= this.#gridLength
    } else {
      this.#height += this.#gridLength
      if (this.#height > MAX_HEIGTH_DIMENSION) this.#height -= this.#gridLength
      this.#width += this.#gridLength
